...
> 這是[一個工程師的財務自白](https://www.youtube.com/@luge.finance)播客系列的配套工具，EP17 有完整示範。
...
| 平台 | 版本 | 下載 |
|------|------|------|
| Windows 10 / 11 | v0.5.2 | [asset-dashboard_0.5.2_x64-setup.exe](https://github.com/Shiauka/asset-dashboard-tauri/releases/download/v0.5.2/asset-dashboard_0.5.2_x64-setup.exe) |
...
| 分頁 | 說明 |
|------|------|
| 資產分布 | 圓餅圖顯示資產桶配置現況，點擊可展開個股明細；條形圖比對目標比例 vs 實際比例。資產桶可自訂：新增／刪除／改名／改色／排序，預設五桶、用不到的桶可刪掉（不會留空桶） |
| 資產走勢 | 以本機 JSON 快照繪製歷史資產曲線 |
| 績效分析 | 時間加權報酬率（TWR）計算，區隔投入資金與投資報酬；報酬率同時顯示**損益金額**（期末資產−期初資產−當期淨存入），含累計 / 今年 / 近一年，以及各年度、各月度的損益金額 |
| 退休規劃 | 設定目標金額、退休年齡、月投入，自動計算達成進度與所需年化報酬率，含退休軌跡圖；年度／月度里程碑以「**計畫 vs 實際**」呈現（計畫值錨定開始追蹤日，對照實際快照算出領先/落後計畫的金額） |
| 再平衡分析 | 計算各標的缺口（萬元 / 股數），支援台幣 / 美金帳戶分開檢視；偏離警示門檻可自訂（預設 5%，依個人風險偏好調整）；新資金分配助手可選擇「另外從防禦桶提領」一筆金額併入分配池（內部重新分配，不虛增總資產，低於防禦桶目標時會自動優先補回） |
| 持倉明細 | 管理股票與現金帳戶，自訂目標配置比例；可切換「損益視圖」——顯示每檔持倉的均攤成本（WAC）、未實現損益金額與損益百分比 |
| 交易紀錄 | 買入、賣出、現金轉帳、**股息收入**完整記錄，支援編輯與刪除（自動回溯調整歷史快照） |
...
- **資料儲存**：本機 JSON 檔，存放在使用者指定的根目錄（月份化快照 `snapshots/YYYY-MM.json` + 共用交易記錄 `transactions.json`）
...
- **`features.test.ts` — 新功能正確性**：涵蓋均攤成本 WAC 計算（加權、手續費、部分賣出）、股息交易 apply / reverse / 快照回推、稅務損益 by-year 統計、TWR 歷史匯率方向性驗證；以及 `new_position` 重複建立同一標的的股數累計回歸測試（v0.5.1 修正）。
